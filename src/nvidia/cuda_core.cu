        ...
        uint64_t startNonce, uint32_t * __restrict__ d_input )
{
    ...
    const int sub = threadIdx.x & 3; //0,1,2,3...
    const int sub2 = sub & 2;  //0,0,2,2....
    ...
    for ( i = start; i < end; ++i )
    {
		...
		for (int x = 0; x < 2; ++x)
		{
			...
			d[x] = a ^
				...
				t_fn1((x_1 >> 8) & 0xff) ^
				t_fn2((x_2 >> 16) & 0xff) ^
				t_fn3((x_3 >> 24));  //aes The bytes 0..31 of the Keccak final state are interpreted as an AES - 256 key[AES]


			//XOR_BLOCKS_DST(c, b, &long_state[j]);
			t1[0] = shuffle<4>(sPtr, sub, d[x], 0);  //,site_t group_n, uint32_t* ptr, uint32_t sub, const int val,c onst uint32_t src
			                                         //	sptr[sub] = d[x];
				                                     //return ptr[src & (group_n - 1)];
...
#pragma unroll
			...
			*((uint64_t *)t2) += sub2 ? (*((uint64_t *)t1) * *((uint64_t*)zz)) : __umul64hi(*((uint64_t *)t1), *((uint64_t*)zz));
			...
			const uint32_t tmp2 = ((uint8_t*)(&pip))[1]; 
			static const uint32_t table2 = 0x75310; 
			const uint32_t index2 = (((tmp2 >> 3) & 6) | (tmp2 & 1)) << 1; 
			((uint8_t*)(&pip))[1] = tmp2 ^ ((table2 >> index2) & 0x33); 

			storeGlobal32(long_state + j, sub == 0 ? pip : res);
    ...
    }/// this needs to operate on differnt data to the first vairant so cannot be directly copied
}
...
void cryptonight_core_gpu_hash(nvid_ctx* ctx, uint64_t nonce)
{
