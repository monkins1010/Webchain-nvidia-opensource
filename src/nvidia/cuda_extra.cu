...
__global__ void cryptonight_extra_gpu_prepare( int threads, uint32_t * __restrict__ d_input, uint32_t len, uint64_t startNonce, uint32_t * __restrict__ d_ctx_state, uint32_t * __restrict__ d_ctx_state2, uint32_t * __restrict__ d_ctx_a, uint32_t * __restrict__ d_ctx_b, uint32_t * __restrict__ d_ctx_key1, uint32_t * __restrict__ d_ctx_key2 )
{
	...
	uint64_t nthread = (blockDim.x * blockIdx.x + threadIdx.x);
//    __shared__ uint32_t sharedMemory[1024];
	...
	uint8_t *ipp = (uint8_t*)&input[0];

    memcpy( input, d_input, len );
	
	uint64_t nonce = startNonce + nthread;
	uint8_t *nonp = (uint8_t*)&nonce;
    ...
    cn_keccak( (uint8_t *)input,len, (uint8_t *) ctx_state );   // was inpout and len
...
{
#define htonl(n) (((((uint64_t)(n) & 0xFF)) << 24) | \
((((uint64_t)(n) & 0xFF00)) << 8) | \
((((uint64_t)(n) & 0xFF0000)) >> 8) | \
((((uint64_t)(n) & 0xFF000000)) >> 24))
...
int cryptonight_extra_cpu_init(nvid_ctx *ctx, xmrig::Algo algo, size_t hashMemSize)
{
    ...
    */
    ...
    CUDA_CHECK(ctx->device_id, cudaMalloc(&ctx->d_input, 128 * sizeof (uint32_t ) )); //0x4000
...
void cryptonight_extra_cpu_prepare(nvid_ctx* ctx, uint64_t startNonce, xmrig::Algo algo)
{
	...
	//printf("start nonce in prepair %016x \n\n", startNonce);
...
void cryptonight_extra_cpu_final(nvid_ctx* ctx, uint64_t startNonce, uint64_t target, uint32_t* rescount, uint64_t *resnonce, xmrig::Algo algo)
{
	...
	//printf("cpu final resnonce[0] before find = %016X\n\n", resnonce[0]);
