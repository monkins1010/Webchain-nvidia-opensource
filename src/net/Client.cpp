...
#   else
   ...
   char data[65];
    ...
    data[64] = '\0';
#   endif
    const size_t size = snprintf(m_sendBuf, sizeof(m_sendBuf), "{\"id\":%" PRIu64 ",\"jsonrpc\":\"2.0\",\"method\":\"submit\",\"worker\":\"%s\",\"params\":{\"id\":\"%s\",\"job_id\":\"%s\",\"nonce\":\"%s\",\"result\":\"%s\"}}\n",
                                 m_sequence, m_url.password(),m_rpcId.data(), result.jobId.data(), nonce, data);
...
void Client::login()
{
	...
	doc.AddMember("worker", rapidjson::StringRef(m_url.password()), allocator);
